...
const OSCParser = api.handle("OSCParser", .{
    .fields = &.{
        mustField(.{ .path = "kind", .name = "command", .doc = "The last command parsed, or `invalid` if `end` has not said yes." }),
        mustField(.{ .path = "window_title", .name = "windowTitle", .doc = "OSC 0 and OSC 2: the window title. ghostty does not decode it. Under title mode 0 the bytes are hex, and otherwise they are UTF-8 or latin1 depending on how the terminal was set up, which only the embedder knows." }),
        mustField(.{ .path = "icon_name", .name = "icon", .doc = "OSC 1: the icon name. Not well defined by any specification, and ghostty itself ignores it." }),
        mustField(.{ .path = "pwd_value", .name = "pwd", .doc = "OSC 7: the shell's working directory, as a `file://` URL. ghostty does not check that it is one, and neither does this." }),
        mustField(.{ .path = "hyperlink_uri", .name = "hyperlinkURI", .doc = "OSC 8: the link target." }),
        mustField(.{ .path = "hyperlink_id", .name = "hyperlinkID", .doc = "OSC 8: the link's `id=` parameter, empty when it carried none. Two runs sharing an id are one link even when they are not adjacent." }),
        mustField(.{ .path = "notification_title", .name = "notificationTitle", .doc = "OSC 9 and OSC 777: the notification title." }),
        mustField(.{ .path = "notification_body", .name = "notificationBody", .doc = "OSC 9 and OSC 777: the notification body." }),
        ...
        mustField(.{ .path = "clipboard_data", .name = "clipboardData", .doc = "OSC 52: the base64 payload to put on the clipboard, or a bare `?` when the program is asking to read the clipboard rather than to write it." }),
        mustField(.{ .path = "clipboard_kind", .name = "clipboardSelection", .doc = "OSC 52: which selection the request names, as the protocol's own character (`c` for clipboard, `p` for primary, `s` for the configured default), or zero for any other command." }),
        mustField(.{ .path = "mouse_shape", .name = "mouseShape", .doc = "OSC 22: the pointer shape, usually a W3C CSS cursor name. ghostty parses whatever string is given without validating it." }),
        mustField(.{ .path = "prompt_action", .name = "semanticPromptAction", .doc = "OSC 133: which prompt boundary this sequence marks." }),
        mustField(.{ .path = "prompt_options", .name = "semanticPromptOptions", .doc = "OSC 133: the raw, unvalidated option string that followed the action." }),
        mustField(.{ .path = "progress_state", .name = "progressState", .doc = "OSC 9;4: what the program says about its progress." }),
        mustField(.{ .path = "progress", .name = "progressValue", .doc = "OSC 9;4: percent complete, or -1 when the report carried no percentage." }),
    },
}).context();
