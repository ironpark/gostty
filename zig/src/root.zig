...
// -- System hooks -------------------------------------------------------------
//
// ghostty's `sys` namespace holds process-global function pointers for the two
// things the terminal cannot do on its own in a library build: decoding PNG
// bytes (a Kitty graphics transmission with `f=100`) and drawing secure
// entropy (a Kitty clipboard grant password). Both default to "unsupported":
// a PNG transmission is refused and a grant that needs a password fails.
//
...
// before returning. A zigo callback signature is a raw C signature, so
// handing the bytes over directly would split every slice into a pointer and
// a length and leave a borrow in Go's hands.
