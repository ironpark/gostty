...
func TestFontSizeIsScaledByTheDisplay(t *testing.T) {
	...
	tab := &terminalTab{dsf: 2}
	tab.settings.families = families
	tab.settings.size = fonts.DefaultSize
	tab.applyFont()

	if tab.fonts.Size != fonts.DefaultSize*2 {
		t.Errorf("face size = %v, want %v (%v at 2x)", tab.fonts.Size, fonts.DefaultSize*2, fonts.DefaultSize)
	}
	if tab.settings.size != fonts.DefaultSize {
		t.Errorf("the settings size became %v; it should stay in the units the user chose", tab.settings.size)
	}
	if !tab.relayout {
...
func TestBitmapFallbackFollowsTheDisplay(t *testing.T) {
	...
	tab.settings.size = fonts.DefaultSize
	tab.applyFont()
	if tab.fonts.FamilyName != "bitmap" {
		t.Fatalf("family = %q, want the bitmap fallback with no families to choose from", tab.fonts.FamilyName)
	}
	one := fonts.Load(nil, fonts.DefaultSize)
	if tab.fonts.Scale <= one.Scale {
		t.Errorf("scale at 2x is %v, want more than %v", tab.fonts.Scale, one.Scale)
	}
}
