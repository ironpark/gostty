...
func (win *window) settingsAdjust(row, delta int) error {
	...
	case ui.SettingFont:
		...
		win.setFont(wrap(win.settings.family+delta, len(win.settings.families)), win.settings.size)
		return nil
	case ui.SettingTheme:
		return win.setTheme(wrap(win.settings.theme+delta, ui.ThemeCount()))
	case ui.SettingCat:
		win.setCatMode(win.settings.cat.Step(delta))
		return nil
	default:
		win.setFont(win.settings.family, win.settings.size+float64(delta))
		return nil
	}
}
...
func (win *window) settingsValues() ui.SettingsValues {
	return ui.SettingsValues{
		win.fontLabel(), fmt.Sprintf("%.0f px", win.settings.size), win.themeLabel(), win.catLabel(),
	}
}

func (win *window) themeLabel() string {
	return fmt.Sprintf("%s  (%d/%d)", ui.ThemeAt(win.settings.theme).Name, win.settings.theme+1, ui.ThemeCount())
}
...
func (win *window) fontLabel() string {
	...
	label := fmt.Sprintf("%s  (%d/%d)", family.Name, win.settings.family+1, len(win.settings.families))
	if len(have) == 0 {
...
func (win *window) catLabel() string {
	tab := win.current()
...
func (win *window) setFont(family int, size float64) {
	...
	family = win.settings.clampFamily(family)
	if family == win.settings.family && size == win.settings.size {
		return
	}
	win.settings.family, win.settings.size = family, size
	win.applyFont()
}
...
func (win *window) applyFont() {
	win.settings.loadFonts(win.dsf)
	for _, tab := range win.tabs {
		tab.fontsChanged()
	}
}
